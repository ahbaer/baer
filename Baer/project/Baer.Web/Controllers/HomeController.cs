...
namespace Baer.Web.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            DataView dvChange = DbHelper.ExecuteToDataView("select top 11 Description,(case when F_Type='Create' then '新增' when F_Type='Update' then '修改' else '删除' end) F_Type,F_Date from Sys_Log where F_Type in ('Create','Update','Delete') order by F_Date desc");
            List<ShowChange> changes = new List<ShowChange>();
            foreach (DataRowView drv in dvChange)
            {
                ShowChange change = new ShowChange()
                {
                    ChangeInfo = Convert.ToString(drv["Description"]),
                    Type = Convert.ToString(drv["F_Type"]),
                    CreatorTime = Convert.ToDateTime(drv["F_Date"]).ToString("yyy-MM-dd")
                };
        ...
        public ActionResult GetInventoryJson()
        {
            ...
            DataView dvInventory = DbHelper.ExecuteToDataView("select a.WareName,sum(b.Weight) Weight from WareHouse a inner join Inventory b on a.Id=b.WareId group by a.Id,a.WareName,a.CreatorTime order by Weight desc");
        ...
        public ActionResult GetInventoryGroupJson()
        {
            ...
            string inGroupSql = @"
                select 
	                top 12 sum(Weight) Weight, cast(year(CreatorTime) as varchar(4))+'-'+right('00' +cast(month(CreatorTime) as varchar(2)),2) Date 
                from Inventory 
                group by year(CreatorTime),month(CreatorTime) order by year(CreatorTime) desc,month(CreatorTime) desc";
            string outGroupSql = @"
                select 
	                top 12 sum(Weight) Weight, cast(year(CreatorTime) as varchar(4))+'-'+right('00' +cast(month(CreatorTime) as varchar(2)),2) Date 
                from InventoryOut 
